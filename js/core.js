...
class Game {
  ...
  handleTutorialTouch(x, y) {
    ...
    // 任意步骤可跳过教程（最后一步除外）
    if (step < 3 && this.tutorialSkipBtn && this.hit(x, y, this.tutorialSkipBtn)) {
      this.finishTutorial();
      return;
    }
    ...
    if (step === 0) {
      if (this.heldIngredient) {
        ...
        const pan = this.pans.find(p => !p.pancake && Math.hypot(x - p.x, y - p.y) < 45);
        if (pan) this.tutorialPlaceBatter(pan);
        return;
      }
      const btn = this.buttons.find(b => b.id === 'batter');
      if (btn && this.hit(x, y, btn)) { this.onButton(btn); return; }
      // 直接点空锅位时给出引导提示
      const emptyPan = this.pans.find(p => !p.pancake && Math.hypot(x - p.x, y - p.y) < 45);
    ...
    if (step === 1) {
      for (const pan of this.pans) {
        ...
        if (p.phase === 'first' && p.needsFlip) {
          const wasPerfect = p.state === 'perfect';
          p.flip();
          this.spawnText(pan.x, pan.y - 50, wasPerfect ? '完美翻面!' : '翻面!', wasPerfect ? '#FFD700' : '#42A5F5');
          this.spawnParticles(pan.x, pan.y, wasPerfect ? '#FFD700' : '#42A5F5', 4, 8);
          this.tutorial.step = 2; // 关键修复：翻面后进入上菜步骤（此前缺失导致死锁）
  ...
  handleTutorialTouchEnd() {
    ...
    if (this.heldIngredient) {
      const ing = this.heldIngredient;
      for (const pan of this.pans) {
        if (Math.hypot(this.touchX - pan.x, this.touchY - pan.y) >= 45) continue;
        if (ing.type === 'batter' && !pan.pancake) { this.tutorialPlaceBatter(pan); return; }
    ...
    if (this.tutorial.step !== 2 || !this.heldPancake) return;
    const target = this.customers.find(c => c.state === 'waiting' && this.hit(this.touchX, this.touchY, {
      x: c.x - c.width / 2, y: c.y - c.height / 2, w: c.width, h: c.height
    }));
  ...
  tutorialPlaceBatter(pan) {
    const ing = this.heldIngredient;
    if (!ing || ing.type !== 'batter' || pan.pancake) return;
    for (const [res, need] of Object.entries(ing.need)) { this.resources[res] -= need; }
    pan.pancake = new Pancake(this.getCookTime(), this.getSide2Time());
    pan.pancake.x = pan.x; pan.pancake.y = pan.y;
    this.spawnParticles(pan.x, pan.y, '#FFF', 3, 5);
    this.heldIngredient = null;
  ...
  renderTutorial(ctx) {
    ...
    // 对话框：步骤0~2固定在底部（不挡顾客和锅位），步骤3居中
    const boxW = Math.min(w - 40, 340);
    const boxH = step === 3 ? 196 : 128;
    const boxX = (w - boxW) / 2;
    const boxY = step === 3 ? (h - boxH) / 2 - 30 : h - boxH - 26;
    ...
    ctx.fillText((step + 1) + ' / 4', boxX + boxW - 12, boxY + 10);
    ...
    lines.forEach((line, i) => { ctx.fillText(line, w / 2, boxY + 16 + i * 23); });
    ...
    // 跳过教程按钮（步骤0~2显示）
    if (step < 3 && this.tutorialSkipBtn) {
